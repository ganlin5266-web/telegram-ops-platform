...
同一仓库内的独立 React + TypeScript + Vite 前端。后端继续使用根目录 Fastify/PostgreSQL 服务。当前开放总览、用户运营中心、积分汇总与流水、邀请关系、授权审计及人工调整积分。没有生产 Mock、默认账号或浏览器数据库连接。
...
401 或 CSRF 失效清空会话、范围与数据。会话/范围与写操作的 403 清空当前范围数据及权限并提示重新加载授权范围；详情查询区域的 403 只在该区域提示无权限。后端仍执行最终鉴权。范围和详情请求携带本地代数标识，切换时同步失效，旧请求返回后不能写入新范围。
...
当前未提供 CSV、运营指标、品牌/Bot 创建、管理员管理或其他后续模块；依赖未来服务端管理 API。

## 第 2 批：用户运营中心

`src/operations/UsersCenter.tsx` 管理用户搜索、筛选、排序与精确总数；`panels.tsx` 提供概览、积分流水、邀请和审计区域；`query.tsx` 统一处理 350ms 防抖、请求失效、区域错误和游标历史。

搜索使用 Telegram ID 精确匹配或名称前缀匹配。语言筛选必须选择 Telegram 或用户偏好来源，不代表最终发送语言。日期区间为起点包含、终点不包含，浏览器本地时间转 ISO 后发送。筛选与排序变化重置游标；翻页与排序不重复请求 count。失效游标停止重试，提示用户重新加载。

用户列表不逐行请求余额；打开详情后才读取该用户数据，各页签按需加载。切换品牌/Bot 会卸载详情并忽略旧请求。详情区域分别显示加载、空数据、错误和重试，401 统一退出。桌面使用详情抽屉，手机使用全屏详情及筛选面板。

余额、累计获得/消耗、流水和总数保留十进制字符串。累计获得包含退款，人工调整成功后重新读取账户、汇总和流水。邀请数量仅表示现有绑定事实，不表示有效邀请。审计仅向 audit.read 用户请求并显示“当前 Bot 操作日志”：当前 API 不支持按 Telegram 用户查询操作日志，不能把 Bot 日志冒充用户日志。

本批本地验收：67 项组件/API Client 测试及 8 项真实浏览器测试通过，合计 75 项（新增 39 项，保留原有 36 项）。CI 使用 PostgreSQL 17 验证后端与浏览器集成；实际运行结果以 GitHub Actions 对应提交为准。
